...
/**
 * GPS konumundan rota indeksine ve segment içi kesirli pozisyona çevirir.
 * Daha hassas pozisyon takibi için kullanılır.
 */
export function getPreciseRoutePosition(
  lat: number,
  lon: number
): { routeIndex: number; fraction: number; latitude: number; longitude: number } {
  const idx = findNearestRouteIndex(lat, lon);
  const pt = IZBAN_ROUTE_COORDS[idx];
  return { routeIndex: idx, fraction: 0, latitude: pt.latitude, longitude: pt.longitude };
}
